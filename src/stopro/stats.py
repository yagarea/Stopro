...
def print_global_stats(state):
    ...
    session_count = (len(state.log) - (1 if state.is_running else 0))
    if session_count == 0:
        avg_time = get_duration_of_ongoing_session(state.log) if state.is_running else 0
    else:
...
def get_total_time(state):
    return sum([i.total_seconds() for i in get_session_durations(state.log)]) + (
            get_duration_of_ongoing_session(state.log) if state.is_running else 0)


# Print lock status: if it is locked and how long it has been locked
def print_lock_status(state):
    if state.is_locked and not state.is_unlock_allowed:
        print(f"This session is locked. ({format_second(state.locked_for)})")
        return
...
def get_total_time_locked(state):
    return state.total_time_locked
